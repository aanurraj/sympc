...
@pytest.mark.parametrize("security", ["semi-honest", "malicious"])
def test_prime_xor(get_clients, security) -> None:
    parties = get_clients(3)
    protocol = Falcon(security)
    session = Session(protocol=protocol, parties=parties)
    SessionManager.setup_mpc(session)
    ...
    b_sh1 = torch.tensor([17], dtype=torch.uint8)
    b_sh2 = torch.tensor([8], dtype=torch.uint8)
    b_sh3 = torch.tensor([43], dtype=torch.uint8)
